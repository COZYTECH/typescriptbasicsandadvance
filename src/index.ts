...
import { Task, Status } from "./model/user.model.js";
...
function updateTaskStatus(task: Task, status: Status) {
  task.status = status;
...
// adding more status using union types

type TaskStatus = Status | "ON_HOLD";

const task2: Task & { extraStatus?: TaskStatus } = {
  id: 2,
...
function printCompletedTasks(tasks: Task[]) {
  tasks.forEach((task) => {
...
function printTasks(tasks: Task[], status?: Status): void {
  tasks.forEach((task) => {
